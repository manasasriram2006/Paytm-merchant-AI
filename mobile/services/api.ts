import type { AtRiskCustomersResponse, CustomerInsight, CustomerRecord, CustomerSegment, CustomerSummary, ForecastResponse, HomeOverview, InventoryAlert, InventoryDataSummary, InventoryResponse, Recommendation } from '../types/business';
...
class ApiService {
  ...
  getCustomerSummary = () => this.request<CustomerSummary>('/api/customers/summary');
  getCustomerInsights = () => this.request<CustomerInsight[]>('/api/customers/insights');
  getCustomerSegments = () => this.request<CustomerSegment[]>('/api/customers/segments');
  getTopCustomers = (limit = 100) => this.request<CustomerRecord[]>(`/api/customers/top?limit=${limit}`);
  getAtRiskCustomers = (limit = 100) => this.request<AtRiskCustomersResponse>(`/api/customers/at-risk?limit=${limit}`);
